...
interface UserStatusRepository extends JpaRepository<UsersStatusEntity,Integer> {
    ...
    @Query(value = "SELECT * from public.user_status where expiryDate<:cdate" ,nativeQuery = true)
    UsersStatusEntity GetFreeTrial(@Param("cdate") Timestamp cdate);
    @Query(value = "SELECT status_id from public.user_status where id:id order by id desc" ,nativeQuery = true)
